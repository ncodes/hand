...
func TestStore_SharesSessionDatabase(t *testing.T) {
	...
	_, err = storagesqlite.NewSessionStoreFromDB(db)
	require.NoError(t, err)
...
func TestStore_UpsertReplacesExistingVectorAndDimension(t *testing.T) {
	...
	replacement.ContentHash = retrieval.VectorContentHash("replacement")
	...
	require.NoError(t, err)
	require.Equal(t, []string{"vec-a"}, matchIDs(result.Matches))
	require.Equal(t, retrieval.VectorContentHash("replacement"), result.Matches[0].Record.ContentHash)
	require.True(t, retrieval.IsVectorRecordStale(result.Matches[0].Record, "current text"))
}
...
func TestStore_ExistingRecordIndexDeleteErrors(t *testing.T) {
	...
	require.NoError(t, db.Exec(`INSERT INTO `+recordsTable+` (
		...
		"text-embedding-test",
		99,
		retrieval.VectorContentHash("vec-different-dimension"),
		[]byte{0, 0, 0, 0},
		time.Now().UTC(),
		time.Now().UTC(),
	).Error)
	...
	require.NoError(t, db.Exec(`INSERT INTO `+recordsTable+` (
		...
		"text-embedding-test",
		1,
		retrieval.VectorContentHash("vec-same-dimension"),
		[]byte{0, 0, 0, 0},
		time.Now().UTC(),
		time.Now().UTC(),
	).Error)
