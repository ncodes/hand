...
import (
	...
	"github.com/wandxy/hand/internal/storage/session"
)

type searchCandidate struct {
	session.CandidateMatch
	Message messages.Message
}

type searchCandidateSet = session.SearchCandidateSet[string, *searchCandidate]

func (candidate *searchCandidate) CandidateMatchRef() *session.CandidateMatch {
	if candidate == nil {
		return nil
	}
...
func (s *SessionStore) ConfigureVectorStore(opts session.VectorStoreOptions) error {
	if s == nil {
	...
	if rerankMax == 0 {
		rerankMax = session.DefaultRerankCandidateLimit
	}
	...
	s.vectors = &session.VectorConfig{
...
func (s *SessionStore) searchMessagesHybrid(
	ctx context.Context,
	id string,
	opts session.SearchMessageOptions,
	query string,
) ([]session.SearchMessageResult, error) {
	candidateLimit := session.HybridRetrievalCandidateLimit(opts)
...
func (s *SessionStore) searchMessagesLexicalCandidates(
	id string,
	opts session.SearchMessageOptions,
	query string,
	limit int,
	...
	addHits := func(sessionID string, msgs []messages.Message) {
		...
		for _, hit := range hits {
			...
			candidates[session.SourceIDForMessage(sessionID, hit.Message.ID)] = &searchCandidate{
				CandidateMatch: session.CandidateMatch{
					SessionID:       sessionID,
...
func (s *SessionStore) searchMessagesVector(
	...
	if err := retrieval.ValidateEmbeddingResult(req, embedding); err != nil {
	...
	result, err := s.vectors.Store.Search(ctx, retrieval.VectorSearchRequest{
		...
		Filter: retrieval.VectorFilter{
			...
			ToolName:        session.NormalizeMatchValue(opts.ToolName),
		},
	})
	if err != nil {
		logSafeError(s.logSearchEvent("vector search failed", id, opts), err).
...
func (s *SessionStore) vectorMatchesToCandidates(
	...
	for idx, match := range matches {
		sessionID, messageID, ok := session.MessageRefFromSourceID(match.Record.SourceID)
		if !ok {
			continue
		}
		...
		row, ok := session.MessageIndexRowForVectorRecord(
			session.MessageIndexRowsFromMessage(sessionID, message),
			match.Record.ID,
		)
		if !ok || !session.MessageIndexRowMatchesSearchOptions(row, opts) {
			continue
		}
...
func searchCandidateKey(candidate *searchCandidate) string {
	...
	return session.SourceIDForMessage(candidate.SessionID, candidate.Message.ID)
}
...
func retrievalCandidateFromSearchCandidate(candidate *searchCandidate) retrieval.Candidate {
	...
	return retrieval.Candidate{
		...
		ID:           session.SourceIDForMessage(candidate.SessionID, candidate.Message.ID),
		SourceKind:   retrieval.SourceKindSessionMessage,
...
func searchResultsFromCandidates(
	...
	sort.SliceStable(sessionIDs, func(i, j int) bool {
		...
		leftScore := session.CandidateRankingScore(left.HasRerank, left.RerankScore, left.FusedScore)
		rightScore := session.CandidateRankingScore(right.HasRerank, right.RerankScore, right.FusedScore)
	...
	results := make([]session.SearchMessageResult, 0, len(sessionIDs))
	for _, sessionID := range sessionIDs {
		...
		hits := make([]session.SearchMessageHit, 0, len(sessionCandidates))
		...
		for _, candidate := range sessionCandidates {
			...
			hits = append(hits, session.SearchMessageHit{
				SessionID:       candidate.SessionID,
		...
		results = append(results, session.SearchMessageResult{
			SessionID:     sessionID,
...
func (s *SessionStore) indexVectors(ctx context.Context, sessionID string, messages []messages.Message) error {
	if s == nil || s.vectors == nil || len(messages) == 0 {
		return nil
	}

	rows := make([]session.MessageIndexRow, 0, len(messages))
	for _, message := range messages {
		rows = append(rows, session.MessageIndexRowsFromMessage(sessionID, message)...)
	}
	inputs := session.VectorInputsFromIndexRows(rows)
	if len(inputs) == 0 {
		return nil
	}
	...
	req := retrieval.EmbeddingRequest{Model: s.vectors.Model, Inputs: embeddingInputs}
	...
	inputByID := make(map[string]session.VectorInput, len(inputs))
	for _, input := range inputs {
		inputByID[input.ID] = input
	}

	records := make([]retrieval.VectorRecord, 0, len(result.Items))
	for _, item := range result.Items {
		input := inputByID[item.ID]
		records = append(records, retrieval.VectorRecord{
...
func (s *SessionStore) deleteVectorRows(ctx context.Context, sourceIDs []string) error {
	...
	sourceIDs = session.UniqueStrings(sourceIDs)
	if len(sourceIDs) == 0 {
		return nil
	}
...
func compareSearchCandidates(left *searchCandidate, right *searchCandidate) int {
	return session.CompareCandidateOrder(
		session.CandidateRankingScore(left.HasRerank, left.RerankScore, left.FusedScore),
		session.CandidateRankingScore(right.HasRerank, right.RerankScore, right.FusedScore),
...
func countSearchResultMessages(results []session.SearchMessageResult) int {
	var count int
