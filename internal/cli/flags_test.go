...
func TestApplyConfigOverrides_AppliesModelVerifyContextLength(t *testing.T) {
	cfg := &config.Config{}
	var cmd *cli.Command
	cmd = &cli.Command{Flags: RootFlags(nil, nil)}
	cmd.Action = func(context.Context, *cli.Command) error {
	...
	err := cmd.Run(context.Background(), []string{"hand", "--model.verify-context-length=false"})

	require.NoError(t, err)
	cfg.Normalize()
	require.False(t, boolValue(cfg.ModelVerifyContextLength))
}
