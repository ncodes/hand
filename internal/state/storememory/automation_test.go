...
func TestMemoryStore_AutomationJobLifecycle(t *testing.T) {
	store := NewStore()
	...
	_, err = store.PatchJob(ctx, state.AutomationJobPatch{ID: testAutomationJobC + "x"})
	require.EqualError(t, err, "automation job id must be a valid auto_ nanoid")
...
func TestMemoryStore_AutomationRunLifecycle(t *testing.T) {
	store := NewStore()
	ctx := context.Background()
	...
	require.EqualError(t, err, "automation run already exists")
	...
	require.NoError(t, err)
	require.Equal(t, []string{generated.ID, testAutomationRunC, testAutomationRunB, testAutomationRunA}, automationRunIDs(list.Runs))
	...
	list, err = store.ListRuns(ctx, state.AutomationRunQuery{
		IDs: []string{testAutomationRunMissing},
	})
	require.NoError(t, err)
	require.Empty(t, list.Runs)

	list, err = store.ListRuns(ctx, state.AutomationRunQuery{JobID: testAutomationJobB})
	require.NoError(t, err)
	require.Empty(t, list.Runs)
...
func TestMemoryStore_AutomationNilStoreErrors(t *testing.T) {
	var store *Store
	ctx := context.Background()
...
func TestMemoryStore_AutomationZeroValueStoreInitializesMapsAndSortsTies(t *testing.T) {
	store := &Store{}
	ctx := context.Background()
	now := time.Date(2026, 7, 5, 9, 0, 0, 0, time.UTC)

	created, err := store.CreateJob(ctx, state.AutomationJob{Enabled: true})
	require.NoError(t, err)
	require.NoError(t, state.ValidateAutomationJobID(created.ID))

	store.automationJobs[testAutomationJobB] = state.AutomationJob{
		ID:        testAutomationJobB,
		Enabled:   true,
		UpdatedAt: now,
	}
	store.automationJobs[testAutomationJobA] = state.AutomationJob{
		ID:        testAutomationJobA,
		Enabled:   true,
		UpdatedAt: now,
	}

	list, err := store.ListJobs(ctx, state.AutomationJobQuery{IDs: []string{testAutomationJobB, testAutomationJobA}})
	require.NoError(t, err)
	require.Equal(t, []string{testAutomationJobA, testAutomationJobB}, automationJobIDs(list.Jobs))

	run, err := store.CreateRun(ctx, state.AutomationRun{JobID: created.ID})
	require.NoError(t, err)
	require.NoError(t, state.ValidateAutomationRunID(run.ID))
	require.Equal(t, state.AutomationRunStatusRunning, run.Status)
}
